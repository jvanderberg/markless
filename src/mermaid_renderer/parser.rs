use crate::mermaid_renderer::ir::{DiagramKind, Direction, Graph, NodeStyle, Subgraph};
use anyhow::Result;
...
type NodeTokenParts = (
    String,
    Option<String>,
    Option<crate::mermaid_renderer::ir::NodeShape>,
    Vec<String>,
);
...
fn edge_meta_from_class_token(token: &str) -> EdgeMeta {
    ...
    let style = if token.contains("..") {
        crate::mermaid_renderer::ir::EdgeStyle::Dotted
    } else {
        crate::mermaid_renderer::ir::EdgeStyle::Solid
    };

    let mut start_decoration = None;
    let mut end_decoration = None;
    if token.starts_with('*') {
        start_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::DiamondFilled);
    }
    if token.ends_with('*') {
        end_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::DiamondFilled);
    }
    if token.starts_with('o') {
        start_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::Diamond);
    }
    if token.ends_with('o') {
        end_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::Diamond);
    }
    ...
    if token.contains('|') {
        if arrow_start {
            arrow_start_kind = Some(crate::mermaid_renderer::ir::EdgeArrowhead::OpenTriangle);
        }
        if arrow_end {
            arrow_end_kind = Some(crate::mermaid_renderer::ir::EdgeArrowhead::OpenTriangle);
        }
    } else {
        if arrow_start {
            arrow_start_kind = Some(crate::mermaid_renderer::ir::EdgeArrowhead::ClassDependency);
        }
        if arrow_end {
            arrow_end_kind = Some(crate::mermaid_renderer::ir::EdgeArrowhead::ClassDependency);
        }
    }
...
fn parse_state_stereotype(
    ...
    let (shape, label_override) = match stereo.as_str() {
        "choice" => (Some(crate::mermaid_renderer::ir::NodeShape::Diamond), None),
        "fork" | "join" => (
            Some(crate::mermaid_renderer::ir::NodeShape::ForkJoin),
            Some(String::new()),
        ),
        _ => (None, None),
    };
...
fn parse_state_note(
    ...
    crate::mermaid_renderer::ir::StateNotePosition,
    String,
    String,
)> {
    let trimmed = line.trim();
...
fn normalize_state_token(
    ...
    String,
    crate::mermaid_renderer::ir::NodeShape,
    Option<String>,
) {
    let trimmed = token.trim();
    if trimmed == "[*]" || trimmed == "*" {
        let (id, shape) = if is_start {
            ...
            *start_counter += 1;
            (id, crate::mermaid_renderer::ir::NodeShape::Circle)
        } else {
            ...
            let id = end_states
                ...
                .clone();
            (id, crate::mermaid_renderer::ir::NodeShape::DoubleCircle)
        };
...
fn parse_sequence_participant(
    ...
    let keywords = [
        ...
        ("actor ", crate::mermaid_renderer::ir::NodeShape::ActorBox),
        (
            "boundary ",
            crate::mermaid_renderer::ir::NodeShape::ActorBox,
        ),
        ("control ", crate::mermaid_renderer::ir::NodeShape::ActorBox),
        ("entity ", crate::mermaid_renderer::ir::NodeShape::ActorBox),
        (
            "database ",
    ...
    let mut rest = None;
    let mut shape = crate::mermaid_renderer::ir::NodeShape::ActorBox;
...
fn ensure_sequence_node(
    ...
    id: &str,
    shape: Option<crate::mermaid_renderer::ir::NodeShape>,
) {
    ...
    if graph.nodes.contains_key(id) {
        ...
        graph.ensure_node(
            id,
            label,
            Some(crate::mermaid_renderer::ir::NodeShape::ActorBox),
        );
    }
}
...
fn parse_sequence_message(
    ...
    String,
    Option<String>,
    crate::mermaid_renderer::ir::EdgeStyle,
    Option<crate::mermaid_renderer::ir::SequenceActivationKind>,
)> {
    let tokens = [
        "-->>+", "->>+", "-->+", "->+", "-->>-", "->>-", "-->-", "->-", "<--+", "<-+", "<--", "<-",
        "-->>", "->>", "-->", "->",
    ...
    for token in tokens {
        if let Some(pos) = line.find(token) {
            ...
            let activation = if token.ends_with('+') {
                Some(crate::mermaid_renderer::ir::SequenceActivationKind::Activate)
            } else if token.ends_with('-') {
                Some(crate::mermaid_renderer::ir::SequenceActivationKind::Deactivate)
            } else {
                None
            };
...
fn parse_sequence_note(
    ...
    crate::mermaid_renderer::ir::SequenceNotePosition,
    Vec<String>,
    String,
)> {
    let trimmed = line.trim();
    ...
    for raw_line in lines {
            ...
            parse_class_relation_line(line)
        {
            ...
            graph.ensure_node(
                ...
                Some(crate::mermaid_renderer::ir::NodeShape::Rectangle),
            );
            graph.ensure_node(
                &right_id,
...
fn normalize_er_cardinality(
    token: &str,
) -> (String, Option<crate::mermaid_renderer::ir::EdgeDecoration>) {
    let trimmed = token.trim();
    match trimmed {
        "||" | "|" => (
            "1".to_string(),
            Some(crate::mermaid_renderer::ir::EdgeDecoration::CrowsFootOne),
        ),
        "o|" | "|o" | "o" => (
            "0..1".to_string(),
            Some(crate::mermaid_renderer::ir::EdgeDecoration::CrowsFootZeroOne),
        ),
        "|{" | "}|" => (
            "1..*".to_string(),
            Some(crate::mermaid_renderer::ir::EdgeDecoration::CrowsFootMany),
        ),
        "o{" | "}o" | "}" | "{" => (
            "0..*".to_string(),
            Some(crate::mermaid_renderer::ir::EdgeDecoration::CrowsFootZeroMany),
        ),
...
fn parse_er_relation_line(
    ...
    Option<String>,
    Option<String>,
    Option<crate::mermaid_renderer::ir::EdgeDecoration>,
    Option<crate::mermaid_renderer::ir::EdgeDecoration>,
    crate::mermaid_renderer::ir::EdgeStyle,
)> {
    ...
    let (sep, style) = if let Some(idx) = relation_part.find("--") {
        (idx, crate::mermaid_renderer::ir::EdgeStyle::Solid)
    } else if let Some(idx) = relation_part.find("..") {
        (idx, crate::mermaid_renderer::ir::EdgeStyle::Dotted)
    } else {
        return None;
    };
    ...
    for raw_line in lines {
        ...
        )) = parse_er_relation_line(line)
        {
            graph.ensure_node(
                &left,
                None,
                Some(crate::mermaid_renderer::ir::NodeShape::RoundRect),
            );
            graph.ensure_node(
                &right,
                None,
...
fn parse_pie_diagram(input: &str) -> Result<ParseOutput> {
    ...
    for raw_line in lines {
        ...
        if let Some((label, value)) = parse_pie_slice_line(line) {
            graph
                .pie_slices
                .push(crate::mermaid_renderer::ir::PieSlice { label, value });
        }
    }
...
fn parse_mindmap_diagram(input: &str) -> Result<ParseOutput> {
    ...
    for raw_line in lines {
        ...
        let shape = match node_type {
            crate::mermaid_renderer::ir::MindmapNodeType::Circle => {
                crate::mermaid_renderer::ir::NodeShape::Circle
            }
            crate::mermaid_renderer::ir::MindmapNodeType::RoundedRect => {
                crate::mermaid_renderer::ir::NodeShape::RoundRect
            }
            crate::mermaid_renderer::ir::MindmapNodeType::Rect => {
                crate::mermaid_renderer::ir::NodeShape::Rectangle
            }
            crate::mermaid_renderer::ir::MindmapNodeType::Hexagon => {
                crate::mermaid_renderer::ir::NodeShape::Hexagon
            }
            crate::mermaid_renderer::ir::MindmapNodeType::Cloud
            | crate::mermaid_renderer::ir::MindmapNodeType::Bang => {
                crate::mermaid_renderer::ir::NodeShape::RoundRect
            }
            crate::mermaid_renderer::ir::MindmapNodeType::Default => {
        ...
        let node = crate::mermaid_renderer::ir::MindmapNode {
            id: id.clone(),
            label: label.clone(),
            level,
            section,
...
fn parse_mindmap_node_token(
    ...
    String,
    String,
    crate::mermaid_renderer::ir::MindmapNodeType,
    Vec<String>,
) {
    ...
    let mut node_type = crate::mermaid_renderer::ir::MindmapNodeType::Default;
...
fn parse_mindmap_shape(
    raw: &str,
) -> Option<(String, crate::mermaid_renderer::ir::MindmapNodeType)> {
    ...
    let flush_pending = |graph: &mut Graph,
        ...
        if let Some(time) = pending_time.take() {
            graph
                .timeline
                .events
                .push(crate::mermaid_renderer::ir::TimelineEvent {
                    time,
...
fn parse_gantt_diagram(input: &str) -> Result<ParseOutput> {
    ...
    for raw_line in lines {
        ...
        if let Some((task_label, meta)) = line.split_once(':') {
            ...
            graph
                .gantt_tasks
                .push(crate::mermaid_renderer::ir::GanttTask {
                    id: node_id.clone(),
                    label: label.to_string(),
                    start,
                    duration,
...
fn parse_gantt_task_meta(
    ...
    let mut status: Option<crate::mermaid_renderer::ir::GanttStatus> = None;
...
fn gantt_status_from_token(token: &str) -> Option<crate::mermaid_renderer::ir::GanttStatus> {
    match token {
        "done" => Some(crate::mermaid_renderer::ir::GanttStatus::Done),
        "active" => Some(crate::mermaid_renderer::ir::GanttStatus::Active),
        "crit" => Some(crate::mermaid_renderer::ir::GanttStatus::Crit),
        "milestone" => Some(crate::mermaid_renderer::ir::GanttStatus::Milestone),
        _ => None,
    }
}
...
fn parse_gitgraph_diagram(input: &str) -> Result<ParseOutput> {
    ...
    graph
        .gitgraph
        .branches
        .push(crate::mermaid_renderer::ir::GitGraphBranch {
    ...
    for raw_line in lines {
        ...
        if lower.starts_with("merge ") {
            ...
            let commit = crate::mermaid_renderer::ir::GitGraphCommit {
                id: id.clone(),
                message: Some(format!(
                    "merged branch {} into {}",
                ...
                seq: commit_seq,
                commit_type: crate::mermaid_renderer::ir::GitGraphCommitType::Merge,
                custom_type,
                tags,
                parents,
        ...
        if lower.starts_with("commit") {
            ...
            let commit_type = extract_gitgraph_commit_type(line)
                .unwrap_or(crate::mermaid_renderer::ir::GitGraphCommitType::Normal);
...
fn extract_gitgraph_commit_type(
    line: &str,
) -> Option<crate::mermaid_renderer::ir::GitGraphCommitType> {
    let raw = extract_gitgraph_attr(line, "type")?;
    match raw.to_ascii_uppercase().as_str() {
        "NORMAL" => Some(crate::mermaid_renderer::ir::GitGraphCommitType::Normal),
        "REVERSE" => Some(crate::mermaid_renderer::ir::GitGraphCommitType::Reverse),
        "HIGHLIGHT" => Some(crate::mermaid_renderer::ir::GitGraphCommitType::Highlight),
        _ => None,
    }
}
...
fn parse_c4_diagram(input: &str) -> Result<ParseOutput> {
    ...
    graph.direction = Direction::LeftRight;
    graph.c4 = crate::mermaid_renderer::ir::C4Data::default();
    graph
        .c4
        .boundaries
        .push(crate::mermaid_renderer::ir::C4Boundary {
            id: "global".to_string(),
...
fn process_c4_line(
    line: &str,
    c4: &mut crate::mermaid_renderer::ir::C4Data,
    ...
    if let Some((func, args)) = parse_function_call(line) {
        ...
        if is_c4_boundary(&func_lower) {
            ...
            c4.boundaries.push(crate::mermaid_renderer::ir::C4Boundary {
                id: id.clone(),
                label,
                boundary_type,
                descr,
        ...
        if let Some(rel_kind) = c4_rel_kind_for(&func_lower) {
            ...
            if rel_args.len() >= 3 {
                ...
                c4.rels.push(crate::mermaid_renderer::ir::C4Rel {
                    kind: rel_kind,
                    from,
                    to,
                    label,
            ...
            && let Some(id) = positional.first().cloned()
        {
            ...
            let parent_boundary = boundary_stack.last().cloned().unwrap_or_default();
            c4.shapes.push(crate::mermaid_renderer::ir::C4Shape {
                id,
                label,
                type_label,
                techn,
...
fn c4_shape_kind_for(func_lower: &str) -> Option<crate::mermaid_renderer::ir::C4ShapeKind> {
    ...
    if f.contains("system") {
        ...
        return Some(match (is_ext, is_db, is_queue) {
            (true, true, _) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalSystemDb,
            (true, _, true) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalSystemQueue,
            (true, _, _) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalSystem,
            (false, true, _) => crate::mermaid_renderer::ir::C4ShapeKind::SystemDb,
            (false, _, true) => crate::mermaid_renderer::ir::C4ShapeKind::SystemQueue,
            (false, _, _) => crate::mermaid_renderer::ir::C4ShapeKind::System,
        });
    }
    if f.contains("container") {
        ...
        let is_queue = f.contains("queue");
        return Some(match (is_ext, is_db, is_queue) {
            (true, true, _) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalContainerDb,
            (true, _, true) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalContainerQueue,
            (true, _, _) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalContainer,
            (false, true, _) => crate::mermaid_renderer::ir::C4ShapeKind::ContainerDb,
            (false, _, true) => crate::mermaid_renderer::ir::C4ShapeKind::ContainerQueue,
            (false, _, _) => crate::mermaid_renderer::ir::C4ShapeKind::Container,
        });
    }
    if f.contains("component") {
        ...
        let is_queue = f.contains("queue");
        return Some(match (is_ext, is_db, is_queue) {
            (true, true, _) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalComponentDb,
            (true, _, true) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalComponentQueue,
            (true, _, _) => crate::mermaid_renderer::ir::C4ShapeKind::ExternalComponent,
            (false, true, _) => crate::mermaid_renderer::ir::C4ShapeKind::ComponentDb,
            (false, _, true) => crate::mermaid_renderer::ir::C4ShapeKind::ComponentQueue,
            (false, _, _) => crate::mermaid_renderer::ir::C4ShapeKind::Component,
        });
    }
    None
}

fn kind_uses_techn(kind: crate::mermaid_renderer::ir::C4ShapeKind) -> bool {
    matches!(
        kind,
        crate::mermaid_renderer::ir::C4ShapeKind::Container
            | crate::mermaid_renderer::ir::C4ShapeKind::ContainerDb
            ...
            | crate::mermaid_renderer::ir::C4ShapeKind::Component
            | crate::mermaid_renderer::ir::C4ShapeKind::ComponentDb
...
fn c4_rel_kind_for(func_lower: &str) -> Option<crate::mermaid_renderer::ir::C4RelKind> {
    let f = func_lower.replace('-', "_");
    if f.starts_with("birel") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::BiRel);
    }
    if f.starts_with("rel_u") || f.starts_with("rel_up") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::RelUp);
    }
    if f.starts_with("rel_d") || f.starts_with("rel_down") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::RelDown);
    }
    if f.starts_with("rel_l") || f.starts_with("rel_left") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::RelLeft);
    }
    if f.starts_with("rel_r") || f.starts_with("rel_right") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::RelRight);
    }
    if f.starts_with("rel_b") || f.starts_with("rel_back") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::RelBack);
    }
    if f.starts_with("rel") || f.starts_with("relindex") {
        return Some(crate::mermaid_renderer::ir::C4RelKind::Rel);
    }
    None
}
    ...
    for raw_line in lines {
        ...
        if let Some((label, x, y)) = parse_quadrant_point_coords(line) {
            ...
            graph
                .quadrant
                .points
                .push(crate::mermaid_renderer::ir::QuadrantPoint { label, x, y });
        }
    }
...
fn parse_block_diagram(input: &str) -> Result<ParseOutput> {
    ...
    graph.direction = Direction::LeftRight;
    let (lines, init_config) = preprocess_input(input)?;
    let mut block = crate::mermaid_renderer::ir::BlockDiagram::default();
    ...
    for raw_line in lines {
        ...
        for raw in tokens.drain(..) {
            ...
            if is_space {
                block.nodes.push(crate::mermaid_renderer::ir::BlockNode {
    ...
    for raw_line in lines {
            ...
            && let Some((name, values)) = parse_radar_curve(line)
        {
            ...
            graph.ensure_node(
                ...
                Some(crate::mermaid_renderer::ir::NodeShape::Circle),
            );
        }
    }
    ...
    for raw_line in lines {
        ...
        if let Some((series_kind, label, values)) = parse_xy_series_line_v2(line) {
            graph
                .xychart
                .series
                .push(crate::mermaid_renderer::ir::XYSeries {
                    kind: series_kind,
                    label,
                    values,
                });
...
fn parse_xy_series_line_v2(
    ...
    crate::mermaid_renderer::ir::XYSeriesKind,
    Option<String>,
    Vec<f32>,
)> {
    let lower = line.to_ascii_lowercase();
        ...
        for raw_statement in split_statements(&raw_line) {
            ...
            if let Some((id, label, classes)) = parse_state_alias_line(line) {
                ...
                graph.ensure_node(
                    &id,
                    labels.get(&id).cloned(),
                    state_shape.or(Some(crate::mermaid_renderer::ir::NodeShape::RoundRect)),
                );
                apply_node_classes(&mut graph, &id, &classes);
            ...
            if let Some((left, meta, right, label)) = parse_state_transition(line) {
                ...
                let left_shape = if left_shape == crate::mermaid_renderer::ir::NodeShape::RoundRect
                ...
                let right_shape = if right_shape
                    == crate::mermaid_renderer::ir::NodeShape::RoundRect
            ...
            if let Some((position, target_raw, label)) = parse_state_note(line) {
                ...
                graph
                    .state_notes
                    .push(crate::mermaid_renderer::ir::StateNote {
                        position,
                        target: target.clone(),
                        label,
                    });
...
fn parse_sequence_diagram(input: &str) -> Result<ParseOutput> {
    ...
    let mut labels: HashMap<String, String> = HashMap::new();
    let mut order: Vec<String> = Vec::new();
    let mut open_frames: Vec<crate::mermaid_renderer::ir::SequenceFrame> = Vec::new();
    let mut frames: Vec<crate::mermaid_renderer::ir::SequenceFrame> = Vec::new();
    let mut open_boxes: Vec<crate::mermaid_renderer::ir::SequenceBox> = Vec::new();
    ...
    for raw_line in lines {
        ...
        if let Some((color, label)) = parse_sequence_box_line(line) {
            open_boxes.push(crate::mermaid_renderer::ir::SequenceBox {
                label,
                color,
            ...
            || lower.starts_with("break ")
        {
            let (kind, offset) = if lower.starts_with("opt") {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Opt, 3)
            } else if lower.starts_with("loop") {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Loop, 4)
            } else if lower.starts_with("par") {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Par, 3)
            } else if lower.starts_with("rect") {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Rect, 4)
            } else if lower.starts_with("critical") {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Critical, 8)
            } else if lower.starts_with("break") {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Break, 5)
            } else {
                (crate::mermaid_renderer::ir::SequenceFrameKind::Alt, 3)
            };
            ...
            open_frames.push(crate::mermaid_renderer::ir::SequenceFrame {
                kind,
                sections: vec![crate::mermaid_renderer::ir::SequenceFrameSection {
                    label,
                    start_idx,
                    end_idx: start_idx,
                }],
        ...
        if lower == "else" || lower.starts_with("else ") {
            if let Some(frame) = open_frames.last_mut() {
                ...
                frame
                    .sections
                    .push(crate::mermaid_renderer::ir::SequenceFrameSection {
                        label,
                        start_idx: split_idx,
                        end_idx: split_idx,
                    });
        ...
        if lower == "and" || lower.starts_with("and ") {
            if let Some(frame) = open_frames.last_mut()
                && frame.kind == crate::mermaid_renderer::ir::SequenceFrameKind::Par
            {
                let split_idx = graph.edges.len();
                if let Some(last) = frame.sections.last_mut() {
                    last.end_idx = split_idx;
        ...
        if lower == "option" || lower.starts_with("option ") {
            if let Some(frame) = open_frames.last_mut()
                && frame.kind == crate::mermaid_renderer::ir::SequenceFrameKind::Critical
            {
                let split_idx = graph.edges.len();
                if let Some(last) = frame.sections.last_mut() {
                    last.end_idx = split_idx;
        ...
        if let Some((position, participants, label)) = parse_sequence_note(line) {
            ...
            graph
                .sequence_notes
                .push(crate::mermaid_renderer::ir::SequenceNote {
                    position,
                    participants,
                    label,
        ...
        if lower.starts_with("activate ") {
            ...
            if !id.is_empty() {
                ...
                graph
                    ...
                    .push(crate::mermaid_renderer::ir::SequenceActivation {
                        participant: id,
                        index: graph.edges.len(),
                        kind: crate::mermaid_renderer::ir::SequenceActivationKind::Activate,
                    });
            }
            continue;
        }
            ...
            if !id.is_empty() {
                ...
                graph
                    ...
                    .push(crate::mermaid_renderer::ir::SequenceActivation {
                        participant: id,
                        index: graph.edges.len(),
                        kind: crate::mermaid_renderer::ir::SequenceActivationKind::Deactivate,
                    });
            }
            continue;
        }
...
struct EdgeMeta {
    ...
    arrow_start: bool,
    arrow_end: bool,
    arrow_start_kind: Option<crate::mermaid_renderer::ir::EdgeArrowhead>,
    arrow_end_kind: Option<crate::mermaid_renderer::ir::EdgeArrowhead>,
    start_decoration: Option<crate::mermaid_renderer::ir::EdgeDecoration>,
    end_decoration: Option<crate::mermaid_renderer::ir::EdgeDecoration>,
...
fn parse_edge_meta(arrow: &str) -> EdgeMeta {
    ...
    if trimmed.starts_with('o') {
        start_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::Circle);
        trimmed.remove(0);
    } else if trimmed.starts_with('x') {
        start_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::Cross);
        trimmed.remove(0);
    }

    if trimmed.ends_with('o') {
        end_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::Circle);
        trimmed.pop();
    } else if trimmed.ends_with('x') {
        end_decoration = Some(crate::mermaid_renderer::ir::EdgeDecoration::Cross);
        trimmed.pop();
    }
    ...
    let style = if trimmed.contains('=') {
        crate::mermaid_renderer::ir::EdgeStyle::Thick
...
fn parse_click_line(line: &str) -> Option<(String, crate::mermaid_renderer::ir::NodeLink)> {
    let trimmed = line.trim();
    let lower = trimmed.to_ascii_lowercase();
    ...
    Some((
        id,
        crate::mermaid_renderer::ir::NodeLink { url, title, target },
    ))
}

fn parse_node_style(input: &str) -> crate::mermaid_renderer::ir::NodeStyle {
...
fn parse_edge_style(input: &str) -> crate::mermaid_renderer::ir::EdgeStyleOverride {
    let mut style = crate::mermaid_renderer::ir::EdgeStyleOverride::default();
    for part in input.split(',') {
        let mut kv = part.splitn(2, ':');
        let key = kv.next().unwrap_or("").trim();
        let value = kv.next().unwrap_or("").trim();
...
fn split_asymmetric_label(
    token: &str,
) -> Option<(String, String, crate::mermaid_renderer::ir::NodeShape)> {
    let trimmed = token.trim();
    if trimmed.contains('[') {
        return None;
    }
...
fn split_id_label(token: &str) -> Option<(&str, String, crate::mermaid_renderer::ir::NodeShape)> {
...
fn parse_shape_from_brackets(raw: &str) -> (String, crate::mermaid_renderer::ir::NodeShape) {
    let trimmed = raw.trim();
    if trimmed.starts_with("[/") && trimmed.ends_with("/]") {
        return (
            strip_quotes(&trimmed[2..trimmed.len() - 2]),
...
fn parse_shape_from_parens(raw: &str) -> (String, crate::mermaid_renderer::ir::NodeShape) {
    let trimmed = raw.trim();
    if trimmed.starts_with("(((") && trimmed.ends_with(")))") {
        return (
...
fn parse_shape_from_braces(raw: &str) -> (String, crate::mermaid_renderer::ir::NodeShape) {
    let trimmed = raw.trim();
    if trimmed.starts_with("{{") && trimmed.ends_with("}}") {
        return (
            strip_quotes(&trimmed[2..trimmed.len() - 2]),
...
mod tests {
    use super::*;
    use crate::mermaid_renderer::ir::DiagramKind;
    ...
    fn parse_edge_decorations() {
        ...
        assert_eq!(
            ...
            Some(crate::mermaid_renderer::ir::EdgeDecoration::Circle)
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(crate::mermaid_renderer::ir::EdgeDecoration::Cross)
        );
    ...
    fn parse_state_choice_stereotype() {
        ...
        assert_eq!(node.shape, crate::mermaid_renderer::ir::NodeShape::Diamond);
    }
    ...
    fn parse_state_fork_stereotype() {
        ...
        assert_eq!(node.shape, crate::mermaid_renderer::ir::NodeShape::ForkJoin);
    ...
    fn parse_sequence_database_participant() {
        ...
        assert_eq!(node.shape, crate::mermaid_renderer::ir::NodeShape::Cylinder);
    }
    ...
    fn parse_sequence_par_sections() {
        ...
        assert_eq!(
            frame.kind,
            crate::mermaid_renderer::ir::SequenceFrameKind::Par
        );
        assert_eq!(frame.sections.len(), 2);
    ...
    fn parse_sequence_critical_sections() {
        ...
        assert_eq!(
            frame.kind,
            crate::mermaid_renderer::ir::SequenceFrameKind::Critical
        );
        assert_eq!(frame.sections.len(), 2);
        assert_eq!(frame.sections[0].label.as_deref(), Some("ok"));
