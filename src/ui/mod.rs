...
fn render_toc(model: &Model, frame: &mut Frame, area: Rect) {
    ...
    let items: Vec<Line> = headings
        ...
        .map(|(i, h)| {
            ...
            let base_style = style::style_for_line_type(&LineType::Heading(h.level));
...
fn render_document(model: &mut Model, frame: &mut Frame, area: Rect) {
    ...
    if !model.image_protocols.is_empty() {
        for img_ref in model.document.images() {
            let Some((protocol, img_width, img_height)) = model.image_protocols.get_mut(&img_ref.src) else {
                continue;
            };
...
mod tests {
    ...
    fn test_render_with_real_image_creates_protocol() {
        ...
        model.image_protocols.insert("test_image.png".to_string(), (protocol, 52, 26));
    ...
    fn test_image_reserves_fixed_height_in_document() {
        ...
        // Find the image line
        let image_line_idx = lines.iter().position(|l| {
            matches!(l.line_type(), LineType::Image)
        });
        ...
        let image_lines: Vec<_> = lines.iter()
    ...
    fn test_image_size_stable_during_scroll() {
        ...
        let buffer = terminal.backend().buffer();
        let red_cells_at_pos_0: usize = buffer.content().iter()
            .filter(|c| matches!(c.fg, ratatui::style::Color::Rgb(255, 0, 0))
                || matches!(c.bg, ratatui::style::Color::Rgb(255, 0, 0)))
            .count();
        ...
        let buffer = terminal.backend().buffer();
        let red_cells_at_pos_2: usize = buffer.content().iter()
            .filter(|c| matches!(c.fg, ratatui::style::Color::Rgb(255, 0, 0))
                || matches!(c.bg, ratatui::style::Color::Rgb(255, 0, 0)))
            .count();
    ...
    fn test_image_clips_not_resizes_when_scrolling() {
        ...
        assert!(
            width_at_scroll_0 > 0,
            "Image should be visible at scroll 0"
        );
    ...
    fn test_image_scrolls_off_top_of_screen() {
        ...
        let red_cell_count: usize = buffer.content().iter()
            .filter(|c| matches!(c.fg, ratatui::style::Color::Rgb(255, 0, 0))
                || matches!(c.bg, ratatui::style::Color::Rgb(255, 0, 0)))
            .count();
    ...
    fn test_image_partially_visible_when_near_bottom() {
        ...
        assert!(
            ...
            last_red_row, buffer.area.height
        );
    }
    ...
    fn test_image_width_is_65_percent_of_viewport() {
        ...
        assert!(
            max_image_col > 0,
            "Image should render with some width"
        );
        assert!(
            max_image_col <= expected_width + tolerance,
            "Image width ({}) should be <= ~65% of viewport ({}±{})",
            max_image_col + 1, expected_width, tolerance
        );
        assert!(
    ...
    fn test_image_height_based_on_scaled_dimensions() {
        ...
        let wide_img_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/wide_test_image.png");
        ...
        assert!(
            ...
            max_blue_row + 1, expected_rows
        );

        // Cleanup
        std::fs::remove_file(&wide_img_path).ok();
    }
    ...
    fn test_wide_image_renders_at_correct_height() {
        ...
        let wide_img_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/wide_short_image.png");
    ...
    fn test_document_reserves_scaled_image_height() {
        ...
        let wide_img_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/wide_short_image_layout.png");
        let mut wide_img = RgbImage::new(400, 100);
        for pixel in wide_img.pixels_mut() {
            *pixel = Rgb([0, 255, 0]);
        }
    ...
    fn test_image_rescales_on_viewport_resize() {
        ...
        let initial_width = model.image_protocols.get(&fixture_path.display().to_string())
            .map(|(_, w, _)| *w)
        ...
        let new_width = model.image_protocols.get(&fixture_path.display().to_string())
            .map(|(_, w, _)| *w)
